...
class ModelInferenceService(): Service() {
    ...
    companion object Constants{
        val SUCCESS = 1
        val FAILED_NO_CHARACTERS = 0
        val FAILED_OPERATION_CANCELLED = -1
    }
        ...
        override fun handleMessage(msg: Message) {
            try {
                ...
                */
                ...
                val output = processImageInput(this@ModelInferenceService,  resbmp)   // send through python preprocessing pipeline
                ...
                if (coordinates != null && commaIndices != null && coordinates.isNotEmpty()) {
                    ...
                    resultReceiver.send(FAILED_NO_CHARACTERS, Bundle.EMPTY)
                }

                if (!handlerThread.isInterrupted) {
                    Log.i("custom service", "returning bundle")
                    val bundle = Bundle()
                    bundle.putString("data", Gson().toJson(predictedCoordinates))
                    resultReceiver.send(SUCCESS, bundle)

                    Log.i("custom service", "handler thread should have been interrupted")
                } else {
                    Log.i("custom service", "handler thread was interrupted")
                    resultReceiver.send(FAILED_OPERATION_CANCELLED, Bundle.EMPTY)
                }
